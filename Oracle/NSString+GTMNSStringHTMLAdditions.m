...
//  Oracle
//
//  Modified from GTMNSString+HTML (MWFeedParser) by Joshua Oldenburg on 10/20/13.
...
static GTMHTMLEscapeMap gAsciiHTMLEscapeMap[] = {
...
static GTMHTMLEscapeMap gUnicodeHTMLEscapeMap[] = {
...
static int EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
	...
	const GTMHTMLEscapeMap *map = (const GTMHTMLEscapeMap *)mapVoid;
	int val;
...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(GTMHTMLEscapeMap *)table ofSize:(NSUInteger)size escapingUnicode:(BOOL)escapeUnicode {
	...
	if (!buffer || !data2) return nil;
	
	unichar *buffer2 = (unichar *)data2.mutableBytes;
	NSUInteger buffer2Length = 0;
	
	for (NSUInteger i = 0; i < length; ++i) {
		GTMHTMLEscapeMap *val = bsearch(&buffer[i], table, size / sizeof(GTMHTMLEscapeMap), sizeof(GTMHTMLEscapeMap), EscapeMapCompare);
		if (val || (escapeUnicode && buffer[i] > 127)) {
			if (buffer2Length) {
				CFStringAppendCharacters((CFMutableStringRef)finalString, buffer2, buffer2Length);
				buffer2Length = 0;
			}
			if (val) [finalString appendString:[NSString stringWithUTF8String:val->escapeSequence]];
			else [finalString appendFormat:@"&#%d;", buffer[i]];
		} else {
			buffer2[buffer2Length] = buffer[i];
			buffer2Length += 1;
		}
	}
	if (buffer2Length) CFStringAppendCharacters((CFMutableStringRef)finalString, buffer2, buffer2Length);
	return finalString;
}
	...
	do {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				// "standard" sequences
				for (unsigned i = 0; i < sizeof(gAsciiHTMLEscapeMap) / sizeof(GTMHTMLEscapeMap); ++i) {
